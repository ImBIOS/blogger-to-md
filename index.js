    ...
    fs.readFile(backupXmlFile, function (err, data) {
        parser.parseString(data, function (err, result) {
            ...
            posts.forEach(function (post) {
                ...
                fname =
                    sanitize(decodeURI(post["wp:post_name"][0])) ||
                    post["wp:post_id"];
                markdown = "";
                ...
                comments.forEach(function (comment) {
                    ...
                    if (comment["wp:comment_approved"].pop()) {
                        ...
                        cmt.published = comment["wp:comment_date"]
                            ? comment["wp:comment_date"].pop()
                            : "";
                        ...
                        var cont =
                            ...
                            "</div>";
                        cmt.content = comment["wp:comment_content"]
                            ? tds.turndown(cont)
                            : "";

                        cmt.author.name = comment["wp:comment_author"]
                            ? comment["wp:comment_author"].pop()
                            : "";
                        cmt.author.email = comment["wp:comment_author_email"]
                            ? comment["wp:comment_author_email"].pop()
                            : "";
                        cmt.author.url = comment["wp:comment_author_url"]
                            ? comment["wp:comment_author_url"].pop()
                            : "";
        ...
        parser.parseString(data, function (err, result) {
            ...
            if (result.feed && result.feed.entry) {
                ...
                posts.forEach(function (entry) {
                    ...
                    if (
                        entry["app:control"] &&
                        entry["app:control"][0]["app:draft"][0] == "yes"
                    ) {
                        draft = "true";
                    }
                    ...
                    var fname =
                        outputDir + "/" + path.basename(sanitizedTitle) + ".md";
                    console.log(fname);
                    ...
                    tagLabel = entry.category.filter(function (tag) {
                        ...
                        return (
                            tag["$"].term &&
                            tag["$"].term.indexOf("http://schemas.google") == -1
                        );
                    });
        ...
        if (comments.length) {
            ...
            comments.forEach(function (comment) {
                var readableDate =
                    ...
                    comment.published +
                    '">' +
                    moment(comment.published).format("MMM d, YYYY") +
                    "</time>";
